...
class TestDefaultOntology:
    ...
    def test_happy_ontology_evil_mutated_type_names(self) -> None:
    ...
    def test_sad1_ontology_evil_descriptions_not_empty(self) -> None:
    ...
    def test_happy_ontology_evil_required_properties_is_list(self) -> None:
        """Evil: required_properties must be a list."""
        for _type_name, defn in DEFAULT_ONTOLOGY.items():
            assert isinstance(defn["required_properties"], list)

    def test_sad2_ontology_sad_missing_type_not_present(self) -> None:
        """Sad: 'Recipe' is not a default type."""
        assert "Recipe" not in DEFAULT_ONTOLOGY

    def test_happy_ontology_happy_all_types_present(self) -> None:
