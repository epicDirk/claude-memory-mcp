...
def _make_service() -> tuple[Any, MagicMock]:
    ...
    mock_embedder.async_encode = AsyncMock(return_value=[0.1] * 384)
    mock_repo = MagicMock()
    mock_repo.execute_cypher = AsyncMock(return_value=MagicMock(result_set=[]))
    mock_repo.get_bottles = AsyncMock(return_value=[])
    mock_repo.create_node = AsyncMock(return_value={"id": "test-id", "name": "Test"})
