...
def _make_mixin() -> RepositoryQueryMixin:
    ...
    mixin.select_graph = AsyncMock(return_value=mock_graph)  # type: ignore[attr-defined]
    mixin._mock_graph = mock_graph  # type: ignore[attr-defined]
    return mixin
...
class TestQueryTimeline:
    ...
    async def test_happy_with_project_id(self) -> None:
        """Happy: returns entities filtered by project_id."""
        m = _make_mixin()
        node = _entity_node(id="e1", name="Event")
        m._mock_graph.query.return_value = _mock_result([[node]])

        result = await m.query_timeline("2026-01-01", "2026-12-31", project_id="proj-1")
        assert result == [{"id": "e1", "name": "Event"}]
        # Verify project_id was passed in params
        call_args = m._mock_graph.query.call_args
        assert "project_id" in call_args[0][1]

    async def test_happy_without_project_id(self) -> None:
        """Happy: returns entities without project filter."""
        m = _make_mixin()
        node = _entity_node(id="e2", name="Global")
        m._mock_graph.query.return_value = _mock_result([[node]])

        result = await m.query_timeline("2026-01-01", "2026-12-31")
    ...
    async def test_sad_empty_results(self) -> None:
        ...
        m = _make_mixin()
        m._mock_graph.query.return_value = _mock_result([])

        result = await m.query_timeline("2099-01-01", "2099-12-31")
        assert result == []

    async def test_evil_empty_row_filtered(self) -> None:
...
class TestGetTemporalNeighbors:
    ...
    async def test_happy_both_direction(self) -> None:
        """Happy: 'both' returns temporal neighbors."""
        m = _make_mixin()
        n1 = _entity_node(id="n1", name="Before")
        n2 = _entity_node(id="n2", name="After")
        m._mock_graph.query.return_value = _mock_result([[n1], [n2]])

        result = await m.get_temporal_neighbors("e1", direction="both")
        assert len(result) == 2

    async def test_happy_before_direction(self) -> None:
        """Happy: 'before' filters to predecessors."""
        m = _make_mixin()
        n1 = _entity_node(id="n1", name="Predecessor")
        m._mock_graph.query.return_value = _mock_result([[n1]])

        result = await m.get_temporal_neighbors("e1", direction="before")
        assert len(result) == 1
        # Verify query uses correct direction pattern
        query_str = m._mock_graph.query.call_args[0][0]
        assert "<-[r:" in query_str

    async def test_happy_after_direction(self) -> None:
        """Happy: 'after' filters to successors."""
        m = _make_mixin()
        n1 = _entity_node(id="n1", name="Successor")
        m._mock_graph.query.return_value = _mock_result([[n1]])

        result = await m.get_temporal_neighbors("e1", direction="after")
        assert len(result) == 1
        query_str = m._mock_graph.query.call_args[0][0]
        assert "->" in query_str
    ...
    async def test_sad_no_neighbors(self) -> None:
        ...
        result = await m.get_temporal_neighbors("isolated")
        assert result == []
...
class TestCreateTemporalEdge:
    ...
    async def test_happy_creates_edge(self) -> None:
        """Happy: creates temporal edge and returns result."""
        m = _make_mixin()
        m._mock_graph.query.return_value = _mock_result([["PRECEDED_BY", "from-1", "to-1"]])

        result = await m.create_temporal_edge("from-1", "to-1")
        assert result["rel_type"] == "PRECEDED_BY"
        assert result["from_id"] == "from-1"
        assert result["to_id"] == "to-1"

    async def test_happy_with_custom_properties(self) -> None:
        """Happy: custom properties are passed through."""
        m = _make_mixin()
        m._mock_graph.query.return_value = _mock_result([["EVOLVED_FROM", "a", "b"]])

        result = await m.create_temporal_edge(
    ...
    async def test_sad_entity_not_found(self) -> None:
        """Sad: one or both entities not found returns error dict."""
        m = _make_mixin()
        m._mock_graph.query.return_value = _mock_result([])

        result = await m.create_temporal_edge("missing-1", "missing-2")
        assert "error" in result

    async def test_evil_properties_none_gets_created_at(self) -> None:
        """Evil: None properties still gets created_at added."""
        m = _make_mixin()
        m._mock_graph.query.return_value = _mock_result([["PRECEDED_BY", "a", "b"]])

        await m.create_temporal_edge("a", "b", properties=None)
        call_args = m._mock_graph.query.call_args
        props = call_args[0][1]["props"]
        assert "created_at" in props
...
class TestGetBottles:
    ...
    async def test_happy_basic_query(self) -> None:
        """Happy: returns bottles with default filters."""
        m = _make_mixin()
        bottle = _entity_node(id="b1", name="Bottle-2026-01-01")
        m._mock_graph.query.return_value = _mock_result([[bottle]])

        result = await m.get_bottles()
        assert len(result) == 1
...
class TestGetGraphHealth:
    ...
    async def test_happy_populated_graph(self) -> None:
        """Happy: returns health metrics for a graph with data."""
        m = _make_mixin()
        m._mock_graph.query.side_effect = [
        ...
        result = await m.get_graph_health()
    ...
    async def test_sad_empty_graph(self) -> None:
...
class TestGetAllEdges:
    ...
    async def test_happy_returns_edges(self) -> None:
        ...
        result = await m.get_all_edges()
        assert len(result) == 2
        assert result[0] == {"source": "a", "target": "b", "type": "RELATED_TO"}

    async def test_sad_no_edges(self) -> None:
...
class TestGetAllNodeIds:
    ...
    async def test_happy_returns_ids(self) -> None:
        """Happy: returns list of entity IDs."""
        m = _make_mixin()
        m._mock_graph.query.return_value = _mock_result([["id-1"], ["id-2"], ["id-3"]])

        result = await m.get_all_node_ids()
