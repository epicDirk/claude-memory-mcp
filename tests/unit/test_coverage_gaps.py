...
class TestQueryBottlesFilters:
    ...
    async def _call_get_bottles(self, **kwargs: object) -> tuple[str, dict]:
        ...
        class _Repo(RepositoryQueryMixin):
            async def select_graph(self):  # type: ignore[override]
                return mock_graph

        repo = object.__new__(_Repo)
        await repo.get_bottles(**kwargs)  # type: ignore[arg-type]

        call_args = mock_graph.query.call_args
        return call_args[0][0], call_args[0][1]

    async def test_before_date_filter(self) -> None:
        query, params = await self._call_get_bottles(before_date="2026-06-01T00:00:00")
        assert "$before" in query
        assert params["before"] == "2026-06-01T00:00:00"

    async def test_after_date_filter(self) -> None:
        query, params = await self._call_get_bottles(after_date="2025-01-01T00:00:00")
        assert "$after" in query
        assert params["after"] == "2025-01-01T00:00:00"

    async def test_project_id_filter(self) -> None:
        query, params = await self._call_get_bottles(project_id="proj-42")
        assert "$pid" in query
        assert params["pid"] == "proj-42"

    async def test_all_filters_combined(self) -> None:
        query, params = await self._call_get_bottles(
            search_text="hello",
